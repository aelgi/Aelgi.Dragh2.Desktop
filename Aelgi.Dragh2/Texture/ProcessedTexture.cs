...
namespace Aelgi.Dragh2.Texture
{
    public class ProcessedTexture
    {
        ...
        public ProcessedTexture(PixelFormat format, TextureType type, uint width, uint height, uint depth, uint mipLevels, uint arrayLayers, byte[] textureData)
        {
            Format = format;
            Type = type;
            Width = width;
        ...
        public unsafe Veldrid.Texture CreateDeviceTexture(GraphicsDevice gd, ResourceFactory rf, TextureUsage usage)
        {
            ...
            {
                for (uint level = 0; level < MipLevels; MipLevels++)
                {
